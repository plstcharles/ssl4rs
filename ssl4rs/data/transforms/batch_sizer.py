...
class BatchSizer:
    ...
    def __init__(
        ...
        Args:
            batch_size_hint: hint to be used in order to get the batch size. If it's an integer,
                ...
                size.
        """
        assert isinstance(batch_size_hint, (int, str)), \
            f"invalid batch size hint type: {type(batch_size_hint)}"
    ...
    def __call__(
        ...
        if isinstance(self.batch_size_hint, int):
            ...
            assert self.batch_size_hint in batch, \
        ...
        if "batch_size" in batch:
            ...
            if self.throw_if_smaller:
                assert found_batch_size == batch_size, \
                    f"batch sizes mismatch! ({batch['batch_size']} vs {batch_size})"
            else:
                assert found_batch_size <= batch_size, \
...
def get_batch_size(batch: "BatchDictType") -> int:
    ...
    assert "batch_size" in batch, \
        "could not find the mandatory 'batch_size' key in the given batch dictionary!"
    ...
    if isinstance(batch_size, int):
        ...
        assert np.issubdtype(batch_size.dtype, np.integer), \
            f"invalid batch size array type: {batch_size.dtype}"
